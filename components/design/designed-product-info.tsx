...
export default function CreateDesignedProductForm(props: ICreateDesignedProductFormProps) {
  ...
  const onUploadImage = (data: { name: string; designedPrice: number; description: string }) => {
    ...
    submitPreviewList.forEach(image => {
      ...
      const imageRef = ref(storage, `images/${data.name + '-' + image.position + '-' + image.color}`);
      ...
      uploadBytes(imageRef, file).then(snapshot => {
        getDownloadURL(snapshot.ref).then(url => {
          ...
          const submitBlueprint = blueprints.map(blueprint => {
            ...
            if ((blueprint.designInfos && blueprint.designInfos.length === 0) || !blueprint.designInfos)
  ...
  const { mutate: addDesignedProduct, error, isLoading: isLoadingSubmit } = useCreateDesignedProduct(handleCloseDialog);
          ...
          <div className="card-body">
            ...
            >
              <div className="row mb-3">
                <label htmlFor="basic-icon-default-fullname">Tên thiết kế</label>
              ...
              {isEmptyColor && <p className="text-danger">Vui lòng chọn màu áo để in</p>}

              <div className="d-flex justify-content-center pt-4">
                <div className="col-sm-10 d-flex justify-content-around">
                  <button className="btn btn-primary w-30p" color="primary" type="submit">
                    {(isLoading || isLoadingSubmit) && (
                      <span className="spinner-border spinner-border-sm" role="status" aria-hidden="true" />
                    )}
                    Tạo
                  </button>
                  <button className="btn btn-secondary w-30p" onClick={handleCloseDialog} autoFocus type="button">
                    Hủy
                  </button>
                </div>
              </div>
