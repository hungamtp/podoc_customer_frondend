  ...
  const process_align = (position: string, activeObj: fabric.Object) => {
    if (placeHolder && activeObj) {
      ...
      if (position === "left") {
        ...
        activeObj.set({
          top:
            (placeHolder.rect.top || 1) +
            placeHolder.rect.getScaledHeight() / 2 -
            activeObj.getScaledHeight() / 2,
        });
      } else if (position === "center") {
        activeObj.set({
          left:
            (placeHolder.rect.left || 1) +
            placeHolder.rect.getScaledWidth() / 2 -
            activeObj.getScaledWidth() / 2,
        });
      }
    }
  };
