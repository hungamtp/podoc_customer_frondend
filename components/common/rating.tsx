...
export default function UserRating(props: IUserRatingProps) {
  ...
  const totalPages = Math.ceil((ratingData?.elements || filter.pageSize) / filter.pageSize);
                    ...
                    <div className="d-flex justify-content-between">
                      <div className="d-flex align-items-center">
                        ...
                        <div className="flex-1 commentor-detail">
                          <h6 className="mb-0">
                            <a className="text-dark media-heading">{ratingData.user}</a>
                          </h6>
                          <small className="text-muted">{ratingData.date}</small>
                        </div>
                      </div>
                      <ul className="list-unstyled mb-0">
                        <Rating name="size-small" size="medium" value={ratingData.ratingStar} readOnly precision={0.5} />
                      </ul>
                    </div>
                    <div className="mt-3">
                      <p className="text-muted fst-italic p-2 bg-light rounded">{ratingData.comment}</p>
                    </div>
                  </li>
                ))}
            </ul>
