...
export default function AboutPage(props: AboutPageProps) {
  ...
  const { data: blueprints, isLoading: isLoading } = useGetBlueprintByProduct(productId as string);
  const storage = globalThis?.sessionStorage;
  const prev = storage.getItem('prevPath');

  const [renderBlueprint, setRenderBlueprint] = React.useState<Blueprint[]>([]);
  const [isLoadedBlueprints, setIsLoadedBlueprint] = React.useState<boolean>(false);
  ...
  React.useEffect(() => {
    ...
    if (renderBlueprint && blueprints) {
      renderBlueprint?.forEach(renderedBlueprint => {
        if (!renderedBlueprint.tmpFrameImage || renderBlueprint.length < blueprints.length) isLoaded = false;
      });
  ...
  const designCanvas =
    ...
    renderedBlueprints.map(
      blueprint => position === blueprint.position && <DesignCanvas openPreview={openPreview} setIsEdit={setIsEdit} />
    );

  const { data: colors, isLoading: isLoadingColors } = useGetColorsByFactoryAndProductId(factoryId as string, productId as string);
      ...
      if (prev !== '/login')
        blueprints.forEach((blueprint, index) => {
          getBase64FromUrl(blueprint.frameImage).then(tmpImageSrc => {
            setRenderBlueprint(state => [...state, { ...blueprint, tmpFrameImage: tmpImageSrc }]);
          });
        });
      else {
  ...
  return (
    <div className="container-fluid ">
      {isLoadedBlueprints && colors ? (
        <>
          <DesignHeaderLeft openPreview={openPreview} closePreview={closePreview} isPreview={isPreview} isEditPage={false} />

          {isPreview ? <PreviewCanvas isEditPage={false} colors={colors} setIsEdit={setIsEdit} isEdit={isEdit} /> : designCanvas}
        </>
      ) : (
        <LoadingPrompt />
      )}
